...
public class WebDriverFactory {
     ...
     * <p/>
     * 当持续集成服务器安装在非Windows机器上, 没有IE浏览器与XWindows时, 又希望使用真正的浏览器时，
     * 需要使用remote driver调用远程的Windows机器.
    ...
    public static WebDriver createDriver(String driverName) {
        ...
        if (BrowserType.firefox.name().equals(driverName)) {
            ...
            Validate.isTrue(params.length == 4,
                    "Remote driver is not right, accept format is \"remote:localhost:4444:firefox\", " +
                            "but the input is\""
                            + driverName + "\"");
