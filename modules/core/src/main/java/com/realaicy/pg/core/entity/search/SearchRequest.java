...
import com.realaicy.pg.core.entity.search.filter.*;
import com.realaicy.pg.core.entity.search.utils.SearchableConvertUtils;
...
public final class SearchRequest extends Searchable {
    ...
    /**
     * <p/>根据查询参数拼Searchable实现类的对象<br/>
     * 首先组装成为一个完成的查询条件，然后再应用排序和分页。
     * <p/>
     * 查询参数格式：property_op=value 或 customerProperty=value<br/>
     * customerProperty查找规则是：1、先查找domain的属性，2、如果找不到，则查找domain上的SearchPropertyMappings映射规则
    ...
    public SearchRequest(final Map<String, Object> searchParams, final Pageable page, final Sort sort)
        ...
        assemblySearchFilters(searchParams);
        merge(sort, page);
    }

    private void assemblySearchFilters(final Map<String, Object> searchParams) throws SearchException {
    ...
    /**
     * @param key 查询条件key: {@link com.realaicy.pg.core.entity.search.filter.Condition#key}
     * @return Searchable对象
     */
    @Override
    ...
    /**
     * 判断当前查询请求是否包含某一个特定的查询属性
     *
     * @param key 查询属性
     * @return 如果当前查询请求包含某一个特定的查询属性则返回真
     */
    @Override
